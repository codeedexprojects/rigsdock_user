...
import { getBrandAPI } from "../Services/allAPIs";
...
function CategoryProducts() {
  ...
  const handlePriceFilter = async () => {
    ...
    // Filter from current products instead of making API call
  ...
  const handleRatingFilter = async (rating) => {
    ...
    // Filter from current products based on rating
    const filteredProducts = originalProducts.filter(product => {
  ...
  const handleBrandFilter = async (brandId) => {
    ...
    // Filter from current products based on brand
    const filteredProducts = originalProducts.filter(product => {
      return product.brand && product.brand._id === newBrand;
    });
  ...
  const ProductCard = ({ product }) => {
    ...
    return (
      <div
        className="bg-white rounded-lg p-2 h-full flex flex-col text-sm cursor-pointer relative"
        ...
        <div className="h-32 sm:h-40 md:h-48 lg:h-44 xl:h-48 mb-3 bg-white rounded-lg overflow-hidden">
          <div className="relative group w-full h-full">
            <img
              ...
              alt={product.name}
              className="w-full h-full object-cover p-2 rounded-md transition-transform duration-300"
            />
            <button
            ...
            >
              <Heart
                className={`w-3 h-3 sm:w-4 sm:h-4 ${
                  wishlistItems.includes(product._id) ? "fill-red-500 text-red-500" : "text-red-500"
                }`}
              />
            </button>
          </div>
